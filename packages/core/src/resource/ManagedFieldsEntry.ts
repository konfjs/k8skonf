...
export interface ManagedFieldsEntry {
     ...
     * distinguish between managers, even if they share the same name. For example, a status update
     * will be distinct from a regular update using the same manager name. Note that the APIVersion
     * field is not related to the Subresource field and it always corresponds to the version of the
     * main resource.
     */
    subresource?: string;
    /**
     * Time is the timestamp of when the ManagedFields entry was added. The timestamp will also be
     * updated if a field is added, the manager changes any of the owned fields value or removes
     * a field. The timestamp does not update when a field is removed from the entry because another
